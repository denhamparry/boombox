...
var reasons = []string{
	...
	"Tear this legitimate roof off like two dogs caged",
	...
	"Success is my only legitimate option, failure's not",
...
func nextLyric() string {
	if line == len(reasons) {
		line = 0
	}
	line++
	return reasons[(line - 1)]
}
...
func main() {
	...
	router.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(nextLyric())
	})
	log.Println("Listening on port 4000, super secret place at " + os.Getenv("KO_DATA_PATH"))
